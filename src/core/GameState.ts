...
export interface HumanState {
  ...
  chaseFor: number;
  /**
   * 외모를 고르는 시드 난수. **어떤 얼굴이 되는지는 렌더 계층이 정한다.** (§0-4)
   *
   * 여기서 얼굴 이름이나 개수를 알면 순수 로직이 에셋에 묶인다. 정수 하나만
   * 넘기고 `entities/humanAvatar.ts` 가 스킨 수로 접는다. 뽑기를 시스템에서
   * 하는 이유는 §0-5 다 — 렌더에서 뽑으면 같은 시드가 같은 판을 만들지 않는다.
   */
  look: number;
}
