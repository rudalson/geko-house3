...
export const CONFIG = {
  ...
  RUN_COOLDOWN: 3.0,
  /**
   * 제자리 선회 속도 (라디안/초). 1인칭 탱크 조작의 유일한 시선 수단이다. (§25)
   *
   * 3.6 rad/s = 206°/초, 반 바퀴에 0.87초. 이 값이 곧 **밸런스 상수**다 —
   * 쿼터뷰 시절 8방향 입력은 방향 전환 비용이 0이었지만, 지금은 목표를 바꿀
   * 때마다 선회 시간이 사이클에 얹힌다. 낮추면 사이클이 늘어 §3 의 도달 시간이
   * 무너지고, 높이면 화면이 홱홱 돌아 1인칭에서 멀미가 난다.
   * 근거와 실측은 ROADMAP §3-8k.
   */
  TURN_SPEED: 3.6,
  /**
   * 후진 속도 배율. 1인칭에서 뒤는 보이지 않으므로 전진보다 확실히 느려야
   * "뒤로 빼는 건 위험하다" 가 조작으로 읽힌다.
   */
  BACK_MULTIPLIER: 0.55,
  ...
  // ── 짝 도마뱀과 임신 (§24) ─────────────────────────────────────────────
  //
  // 변기와 같은 축의 선택지다 — **지금 손해를 보고 나중에 덩어리로 받는다.**
  // 다만 변기가 "자리를 비운다"는 대가를 치른다면, 이쪽은 **느려지고 커진 채로
  // 계속 싸워야 한다.** 안전한 곳에 숨는 게 아니라 위험을 안고 다니는 쪽이다.
  //
  // 수치 근거는 ROADMAP §3-8h. 보상 비율 하나로 이득비를 조절한다.
  /** 첫 등장까지 걸리는 시간 (초). 초반 루프와 튜토리얼을 방해하지 않는다 */
  MATE_FIRST_APPEAR_SEC: 45,
  /** 산란 후 짝이 다시 나타나기까지 (초) */
  MATE_COOLDOWN_SEC: 75,
  /** 플레이어로부터 최소 이 거리(world u) 밖에 나타난다 — 왕복이 있어야 대가가 된다 */
  MATE_MIN_SPAWN_DIST: 5.0,
  /** 교미 시간 (초). 이동 불가이고 **무적이 아니다** */
  MATE_ANIM_TIME: 2.5,
  /** 임신 지속 시간 (초) */
  MATE_PREGNANCY_TIME: 25,
  /** 임신 중 이동 속도 배율 */
  MATE_SPEED_MUL: 0.78,
  /** 임신 중 히트박스 배율 — 배가 불러 더 잘 걸린다 */
  MATE_HITBOX_MUL: 1.25,
  /**
   * 산란 보너스. 유효 셀 대비 비율 (변기와 같은 BFS 인접 확장).
   *
   * [조정] 0.035(24칸) → 0.008(5칸). 보상의 대부분을 **새끼**에게 넘겼다.
   * 새끼가 스스로 싸는 몫이 이 위에 더해지므로, 24칸을 그대로 두면 §3-8h 가
   * ❌ 로 적어 둔 "짝만 도는 게임" 구간(0.050 = 34칸)을 넘어선다.
   * 0 으로 만들지 않은 이유: 산란하는 순간 화면에서 아무 일도 일어나지 않으면
   * 25초를 기다린 보람이 알 껍데기 하나로 끝난다. 둥지 자리만 남긴다.
   */
  MATE_EGG_BONUS_RATIO: 0.008,

  // ── 새끼 도마뱀 (§24) ──────────────────────────────────────────────────
  //
  // 산란하면 새끼가 태어나 플레이어를 따라다니며 **스스로 싼다.**
  // 즉시 받던 24칸을 시간에 걸쳐 받는 형태로 바꾼 것이라, 총량은 같은 급이고
  // 대신 중첩 손실을 그대로 받는다 (플레이어 똥과 같은 applyPoop 을 쓴다).
  //
  // 수치 근거는 ROADMAP §3-8i. 총 출력 = (수명 − 첫배변) / 간격 + 1 회.
  /** 동시에 존재할 수 있는 새끼 수. 수명 60 < 쿨다운 75 라 보통 1마리다 */
  HATCHLING_MAX: 2,
  /** 수명 (초). 다하면 집을 떠난다 */
  HATCHLING_LIFETIME_SEC: 60,
  /** 태어난 뒤 첫 배변까지 (초). 부화 연출과 겹치지 않게 띄운다 */
  HATCHLING_FIRST_POOP_SEC: 4,
  /** 배변 간격 (초) */
  HATCHLING_POOP_INTERVAL: 10,
  /**
   * 새끼 배변 반경 (셀). 어른(2.3)보다 한참 작다.
   *
   * 이 값에서 새끼 1마리의 총 출력이 나온다: 6회 x 4.0셀 = **24칸** —
   * 예전 산란 보너스(V x 0.035 = 24칸)와 정확히 같은 총량이다. 즉시 받던 것을
   * 60초에 나눠 받는 것이므로, 그동안 청소기가 갉아먹는 만큼이 순수한 손해다.
   ...
   * 선회 속도 (라디안/초).
   *
   * 격자 경로라 진행 방향이 0.5초마다 45~90도씩 계단으로 꺾인다. 목표각을 그대로
   * 대입하면 얼굴이 한 프레임에 홱 돌아가 눈에 띈다. 8.0 이면 90도 꺾는 데 0.2초라
   * 사람이 도는 게 보이면서도 추격이 굼떠 보이지 않는다. 플레이어 선회(3.6)보다
   * 빠른 이유는, 플레이어는 조작감 때문에 느린 것이고 여기는 연출이기 때문이다.
   */
  HUMAN_TURN_SPEED: 8.0,
  /** 발견 시 말풍선이 떠 있는 시간 */
